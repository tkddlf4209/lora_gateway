...
int rak811_p2p_init(int fd){

    // set Lora Mode : 0 LoRaWAN , 1: P2P
    printf("INFO: RAKK811 SET WORK MODE (P2P)\n");
    send_msg(fd,"at+set_config=lora:work_mode:1\r\n");
    sleep(1);

    // set Regoin
    printf("INFO: RAKK811 SET REGION (KR920)\n");
    send_msg(fd,"at+set_config=lora:region:KR920\r\n");
    sleep(1);

    /* init lora config*/
    printf("INFO: RAKK811 INIT P2P CONFIG (922100000:12:0:1:64:20)\n");
    send_msg(fd,"at+set_config=lorap2p:922100000:12:0:1:64:20\r\n");
    sleep(1);

    printf("INFO: RAKK811 GET LoRaP2P Status\n");
    send_msg(fd,"at+get_config=lora:status\r\n");
    read_msg(fd,1000,true); 
...
int read_msg(int fd, int waitMs, bool clear){
    if(clear){
        serialFlush(fd); // 초기화
    }
    ...
    *nb_pkt = 0;
    ...
    while (serialDataAvail(fd) > 0 ) {
            ...
            if ('\n' == uartInput[uartInputIndex]){
                ...
                uartInputIndex = 0;
                memset(uartInput, 0, UART_INPUT_MAX_SIZE+1);
                //break; //zero terminator received
            } else {
                if(uartInput[uartInputIndex] != '\r'){
                    ++uartInputIndex;
                }
